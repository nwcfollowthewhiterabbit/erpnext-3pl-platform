...
Scanner route for picking confirmation:

`/warehouse/picking-confirmation`

Scanner route for putaway:
...
- Minimal scanner-first stocktake page is available at `/warehouse/stocktake` and records counted quantity by container/SKU with linked corrections for deltas.
