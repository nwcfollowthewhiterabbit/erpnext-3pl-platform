...
> Статус: документ относится к Desk-native MVP1 flow. Для короткой инструкции по ручному тестированию клиентом использовать `docs/mvp1-workflow-test-guide-ru.md`. Для актуальных технических имен, URL и ролей использовать `docs/operational-names.md`; для клиентского входа использовать `/desk/3pl-client`.
...
1. Клиент попадает в ограниченный ERPNext Desk workspace `3PL Client`.
...
- клиент остается в ограниченном workspace `3PL Client`;
- нет `Not permitted`;
- нет `Page not found`;
...
- клиент не попадает в нерелевантные Desk-разделы;
