...
- CSV Product Import save creates/updates client products immediately; product saves then synchronize ERPNext Items immediately.
...
| LFR-02 | Product Import to Products/Items | Fixed for CSV imports. Processor remains as recovery, including XLSX processing. | After client uploads a CSV file, products are created/updated and import status changes to `Applied` or `Failed` immediately. | Server Script: `3PL Product Import Immediate Sync`; product saves trigger `3PL Client Product Immediate Sync`. | Live validation asserts import status, client product, and linked Item without calling the processor. | Done |
...
| LFR-05 | Test Suite Design | Improved for MVP live flows. | Live-flow tests prove immediate UI/server behavior; processors remain tested through deployment/recovery paths. | Deployment, client portal, and warehouse operation packs now include immediate-behavior checks. | `validate_site.py`, `validate_warehouse_ops.py`, and Playwright client portal tests. | Done |
...
- XLSX product import remains covered by the recovery processor path; CSV import is immediate in the portal flow.
