...
- Outbound courier parcel / tracking label flow. If the courier number is only a shipment reference, store it on the shipment/outbound document. If the warehouse physically creates a parcel per order, model it as a short-lived outbound Handling Unit and close it after shipping. Barcode input must be scan-first with manual entry fallback for courier labels that do not scan reliably.
