...
from erpnext_3pl.config.project_config import CLIENT_DESK_CUSTOMER, CLIENT_DESK_USER, COMPANY, WAREHOUSE_MANAGER_USER
...
def submit_stock_entry(entry_type, purpose, flow, item_code, qty, source=None, target=None, **extra):
    ...
    entry.client = CLIENT_DESK_CUSTOMER
...
def create_client_product():
    ...
    product.insert()
    require(product.owner == CLIENT_DESK_USER, "Client product was not created by Desk user")

    frappe.set_user("Administrator")
    item_code = sync_product(product.name)
    product.reload()
...
def create_receiving_notice(item_code):
    ...
    notice.insert()
    require(notice.owner == CLIENT_DESK_USER, "Receiving Notice was not created by Desk user")
    return notice.name
...
def receive_goods(notice_name, item_code):
    ...
    require(notice.items[0].received_qty == E2E_QTY, "Receiving Notice received_qty is wrong")
...
def create_shipment_request(item_code):
    ...
    request.insert()
    require(request.owner == CLIENT_DESK_USER, "Shipment Request was not created by Desk user")
    ...
    require(pick_list.client == CLIENT_DESK_CUSTOMER, "Pick List has wrong client")
    require(any(row.container_code == CONTAINER_CODE and row.item_code == item_code and row.qty == E2E_QTY for row in pick_list.locations), "Pick List did not allocate the E2E container")
    container = frappe.get_doc("Three PL Container", CONTAINER_CODE)
...
def pick_pack_ship(request_name, pick_list_name, item_code):
    ...
    frappe.set_user("Administrator")
    picked = sync_pick_list(pick_list.name)
...
def validate_client_visibility(item_code):
    from erpnext_3pl.sync.inventory_balance_snapshots import sync_inventory_balance_snapshots

    frappe.set_user("Administrator")
    sync_inventory_balance_snapshots()

    frappe.set_user(CLIENT_DESK_USER)
    product_name = frappe.db.get_value("Three PL Client Product", {"customer": CLIENT_DESK_CUSTOMER, "client_sku": CLIENT_SKU}, "name")
    ...
    request_name = frappe.db.get_value("Three PL Shipment Request", {"customer": CLIENT_DESK_CUSTOMER, "external_reference": SHIPMENT_REF}, "name")
