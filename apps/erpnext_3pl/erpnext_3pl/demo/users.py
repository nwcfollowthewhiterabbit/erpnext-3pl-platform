...
USERS = [
    ...
    {
        "email": CLIENT_DESK_USER,
        "first_name": "Alpha",
        "last_name": "Client",
        "password": CLIENT_DESK_PASSWORD,
        "roles": ["3PL Client"],
        "user_type": "System User",
        "customer": CLIENT_DESK_CUSTOMER,
        "module_profile": "3PL Client Only",
        "default_workspace": "3PL Client",
        "default_app": None,
    },
]
...
def require_password(user_data):
    if not user_data.get("password"):
        raise RuntimeError(f"Missing password environment variable for demo user: {user_data['email']}")
...
def ensure_user(user_data):
    ...
    user.new_password = require_password(user_data)
    user.save(ignore_permissions=True)
