...
from erpnext_3pl.config.project_config import CLIENT_DESK_USER, COUNTRY, DEMO_CLIENTS, DEMO_ITEMS
...
def ensure_client_product(item_code, item_name, client, client_sku, client_product_name, barcode):
    ...
    product.notes = "Demo product card managed through the client Desk layer."
...
def ensure_inbound_notice():
    ...
    notice.status = "Draft"
    notice.notes = "Demo client notification for receiving and comparison flow."
    notice.append(
        "items",
        {
    ...
    set_owner_after_save(notice, CLIENT_DESK_USER)
    return notice
...
def ensure_inventory_snapshots():
    ...
    for snapshot in snapshots:
        ...
        if should_set_client_owner and doc.is_new():
            doc.owner = CLIENT_DESK_USER
        doc.last_updated = now()
        doc.save(ignore_permissions=True)
        if should_set_client_owner:
            set_owner_after_save(doc, CLIENT_DESK_USER)
...
def ensure_shipment_request():
    ...
    if request.is_new():
        request.owner = CLIENT_DESK_USER
    request.notes = "Demo outbound request created for client Desk testing."
    request.append(
        "items",
        {
            "item_code": "SKU-ALPHA-003",
    ...
    set_owner_after_save(request, CLIENT_DESK_USER)
    return request
...
def ensure_client_instruction(notice_name, item_code="SKU-ALPHA-002", client_sku="ALPHA-002", instruction_type="Accept Difference", instruction_text=None):
    ...
    if instruction.is_new():
        instruction.owner = CLIENT_DESK_USER
    ...
    set_owner_after_save(instruction, CLIENT_DESK_USER)
    return instruction
...
def ensure_additional_alpha_portal_data():
    ...
    ensure_alpha_shipment_request(
        ...
        notes="Demo draft outbound request for client Desk review.",
        items=[
            {
                "item_code": "SKU-ALPHA-003",
                "client_sku": "ALPHA-003",
